...
void LD2410S::read_all_() {
  ...
  this->tx_schedule_.append(SN_READ_CMD);
...
void LD2410S::read_serial_number_() { this->tx_schedule_.append(SN_READ_CMD); }
...
void LD2410S::parse_ack_sn_read_(const uint8_t *data) {
  uint16_t read_position = 0;
  uint16_t sn_length = 0;
  read_seq_data(data, read_position, &sn_length);
  std::string sn = "";
  for (uint16_t i = 0; i < sn_length && i < 8; i++) {
    sn += (char)data[read_position + 1];
  }
  this->publish_serial_number_(sn);
}
void LD2410S::publish_serial_number_(const std::string &sn, bool force_publish) {
#ifdef USE_TEXT_SENSOR
  if (this->serial_number_text_sensor_ != nullptr) {
    if (this->serial_number_text_sensor_->state != sn || force_publish) {
